...
import { InterventionDetector } from '../lib/intervention-detector';
...
const createObserveElementSchema = (returnAction: boolean) => {
  const baseSchema = {
    ...
    description: z
      .string()
      .describe(
        'A brief description of the component and what action can be taken on it. Keep this short and concise.'
      ),
  };
  ...
  if (returnAction) {
    return z.object({
      ...
      method: z
        .string()
        .describe(
          "The suggested method to interact with the element. Valid methods are: click, type, check, uncheck, select, navigate, scroll, press. If no interaction is possible, return 'none'."
        ),
      arguments: z
        .array(z.any())
        .describe(
          "An array of arguments for the method. For 'type', this is the text to type. For 'select', this is the value of the option to select. For 'scroll', this is ['up', 'down', 'left', 'right', 'top', 'bottom', 'near', 'far']. For 'press', this is the key to press."
        ),
    });
  }
...
const createObserveResultSchema = (returnAction: boolean) =>
  z.object({
    elements: z
      .array(createObserveElementSchema(returnAction))
...
export class ObserveHandler implements IObserveHandler {
  ...
  async observe(
    ...
    try {
      ...
      if (
        this.interventionDetector &&
        this.infrastructureProvider?.handleIntervention
      ) {
        try {
          const interventionEvent =
            await this.interventionDetector.detectIntervention(
          ...
          if (interventionEvent) {
            ...
            await this.infrastructureProvider.handleIntervention(
              interventionEvent
            );
          }
  ...
  private mapElementsToResults(
    ...
    return elements
      .map((element) => {
        ...
        const xpath =
          xpathMap[elementId] || `//*[@data-element-id='${elementId}']`;

        const backendNodeId = this.extractBackendNodeId(elementId);
