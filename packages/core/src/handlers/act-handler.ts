...
export class ActHandler implements IActHandler {
  ...
  constructor(
    ...
    this.selfHeal = this.config.features?.selfHeal ?? true;
  ...
  async actFromObserveResult(
    ...
    const actionDescription =
      observe.description || `ObserveResult action (${method})`;
    ...
    try {
      ...
      this.addActionToHistory(actionDescription, true, undefined, {
        method,
        selector,
      });
      ...
      // Track failed action
      this.addActionToHistory(actionDescription, false, error.message, {
        method,
        selector,
      });

      // Check if intervention is required
      const interventionTriggered = await this.checkForIntervention(
        error,
        actionDescription
      );
  ...
  async observeAct(
    ...
    const doObserveAndAct = async (): Promise<ActResult> => {
      ...
      if (observeResults.length === 0) {
        // Track failed action
        this.addActionToHistory(action, false, 'No observe results found', {
          instruction,
        });
  ...
  private async _performPlaywrightMethod(
    ...
    const context: MethodHandlerContext = {
      ...
      initialUrl,
      ...(domSettleTimeoutMs !== undefined ? { domSettleTimeoutMs } : {}),
    };
