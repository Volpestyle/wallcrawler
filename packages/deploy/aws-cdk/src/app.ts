...
const proxyMinContainers = app.node.tryGetContext('proxyMinContainers');
const proxyMaxContainers = app.node.tryGetContext('proxyMaxContainers');
...
new ApplicationServicesStack(app, `${config.projectName}-app-${config.environment}`, {
  ...
  proxyMinContainers: proxyMinContainers ? parseInt(proxyMinContainers) : undefined,
  proxyMaxContainers: proxyMaxContainers ? parseInt(proxyMaxContainers) : undefined,
  customDomain,
  certificateArn,
