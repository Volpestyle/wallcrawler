...
export class DefaultNetworkMonitor implements NetworkMonitor {
  ...
  constructor(
    private options: SettlementOptions = DEFAULT_SETTLEMENT_OPTIONS
  ) {}
  ...
  async initialize(cdpSession: CDPSession): Promise<void> {
    ...
    cdpSession.on(
      ...
      this.handleRequestWillBeSent.bind(this)
    );
    cdpSession.on(
      "Network.responseReceived",
      this.handleResponseReceived.bind(this)
    );
  ...
  isSettled(options: SettlementOptions): boolean {
    ...
    // Don't log every settlement check - too noisy
  ...
  private handleResponseReceived(params: any): void {
    ...
    // Don't log every response - too noisy
  }
  ...
  private handleLoadingFailed(params: any): void {
    ...
    // Log failed requests as they might be important
  ...
  private handleLoadingFinished(params: any): void {
    ...
    // Don't log every finished request - too noisy
  }
