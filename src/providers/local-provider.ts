...
import { createWallCrawlerPage } from '../core/wallcrawler-page';
import { DefaultCDPSessionManager } from '../core/cdp-session-manager';
import { DefaultNetworkMonitor } from '../core/network-monitor';
...
export class LocalProvider implements InfrastructureProvider {
  ...
  async saveState(state: BrowserState): Promise<StateReference> {
    ...
    logger.info('State saved locally', { sessionId: state.sessionId, filepath });
  ...
  async loadState(reference: StateReference): Promise<BrowserState> {
    ...
    logger.info('State loaded from local storage', { sessionId: state.sessionId });
    return state;
  }
  ...
  async loadArtifact(reference: ArtifactReference): Promise<Artifact> {
    const data = reference.type === 'dom' || reference.type === 'trace'
  ...
  async handleIntervention(event: InterventionEvent): Promise<InterventionSession> {
  ...
  private getContentType(type: Artifact['type']): string {
    switch (type) {
      ...
      case 'dom': return 'application/json';
      case 'video': return 'video/webm';
      case 'trace': return 'application/json';
